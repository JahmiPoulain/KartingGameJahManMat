...
public class Aiguille : MonoBehaviour
{
    ...
    [SerializeField] private float normalMaxDisplayedSpeedKmh = 25f;
    [SerializeField] private float turboMaxDisplayedSpeedKmh = 35f;
    ...
    // Valeur affichée de la vitesse en km/h, lissée pour éviter les à-coups
    ...
    // Variation du texte en km/h quand l'aiguille tremble.
    [SerializeField] private float speedTextShakeAmountKmh = 2f;
    ...
    void Update()
    {
        ...
        float currentMaxDisplayedSpeedKmh = Mathf.Lerp(normalMaxDisplayedSpeedKmh, turboMaxDisplayedSpeedKmh, boostBlend);

        // Si le kart est en train de voler, on utilise la vitesse de vol pour le calcul, sinon la vitesse au sol.
        float realSpeedKmh = kart.isFlying ?
                             kart.flightSpeed + kart.currentTurboForce :
                             kart.currentSpeed + kart.currentTurboForce;
        realSpeedKmh = Mathf.Clamp(realSpeedKmh, 0f, currentMaxDisplayedSpeedKmh);

        // Au lieu de faire un changement brutal de la position de l’aiguille, on lisse la transition pour un effet plus fluide
        displayedSpeedKmh = Mathf.Lerp(displayedSpeedKmh, realSpeedKmh, Time.deltaTime * speedSmooth);
        float displayedSpeedRatio = currentMaxDisplayedSpeedKmh > 0f ? displayedSpeedKmh / currentMaxDisplayedSpeedKmh : 0f;
        displayedSpeedRatio = Mathf.Clamp01(displayedSpeedRatio);
        ...
        float angle = zeroSpeedAngle + Mathf.Lerp(0f, maxAngle, displayedSpeedRatio);
        ...
        float shakeIntensity = Mathf.InverseLerp(shakeThreshold, 1f, displayedSpeedRatio);
        ...
        if (speedText != null)
        {
            float textSpeed = displayedSpeedKmh;
            textSpeed += shake * speedTextShakeAmountKmh * shakeIntensity;
            speedText.text = Mathf.RoundToInt(textSpeed).ToString();
        }
