...
public class SoundManager : MonoBehaviour
{
    ...
    [Header("Mixer Group (pour lier � tes options)")]
    ...
    private void Awake()
    {
        ...
        {
            Instance = this;
            DontDestroyOnLoad(gameObject); // Reste actif d'une sc�ne � l'autre
        }
        else
        {
            Destroy(gameObject);
    ...
    // Permet de d�clencher un effet sonore 2D depuis n'importe o� (ex: Checkpoint, Compte � rebours)
    // Dans SoundManager.cs (assure-toi que cette m�thode est publique)
